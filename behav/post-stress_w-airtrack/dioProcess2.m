% version from 8 Mar 2011


function out = dioProcess2(filename, t1, t2, outputfile, day, run)
...
Vn = datenum([zeros(size(diotimes, 1),5) diotimes(:)/TIMESTAMPRATE]);
...
epoch1 = diovals(I1:I2,[11 12 21 22 23]);  % add the timestamp with V
tS1 = tS(I1:I2, :);
...
reward = epoch1(:,1) == 1 & [1; epoch1(1:end-1,1)] == 0 & [1; epoch1(1:end-1,2)] == 0;

% excel fromula =IF(  AND(  OR((N776=1),(I777=1)),  (K777=0),(J777=0)),   1,0) 

goal = zeros(length(epoch1), 7); % reward, index, goal1, goal2, goal3, attempt, error
goal(1, 1:5) = epoch1(1,:); % ???
% ****
...
goal(:,1) = reward;
goal(1,2) = 1;  % index
goal(:,8) = diotimes(I1:I2, :);  % dio timestamp for later calculations
goal(:,9) = day;
goal(:,10) = run;
%length(tS1)
%length(goal)
% casenames = {num2str(1)};
for i = 2:length(epoch1)
    goal(i,3) = or(epoch1(i,3)==1, goal(i-1, 3)==1) & epoch1(i,4)==0 & epoch1(i,5)==0; % arm1 if not arm2 or 3 and currently or previousy in 1
    goal(i,4) = or(epoch1(i,4)==1, goal(i-1, 4)==1) & epoch1(i,3)==0 & epoch1(i,5)==0; % arm2 if not arm1 or 3 and currently or previousy in 2
    goal(i,5) = or(epoch1(i,5)==1, goal(i-1, 5)==1) & epoch1(i,4)==0 & epoch1(i,3)==0; % arm3 if not arm1 or 2 and currently or previousy in 3
