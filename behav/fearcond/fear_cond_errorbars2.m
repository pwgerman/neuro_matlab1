...
%load fear_cond
figfilename = 'Fear_Cond_Phys-freezing.jpg'; % also need to rename the file output at end.

%load fear_cond_KLMNO.mat;

cntrlbar = cntrl; % form colums=days and rows= animals
shockbar = shock;
...
axis([.5 (num_trials+.5) 0 500]);
hlegend = legend('control track', 'shock track');
set(hlegend, 'Box', 'off');
set(gca, 'YTick', (0:100:500));

saveas(gcf,figfilename); % best for direct export to MS PPT
...
print -depsc2 -r600 -painter fear_cond_figure.eps  % -f2 -r600 (adding these should select figure2 and export at 600dpi resolution)
