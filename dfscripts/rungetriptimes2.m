% rungetriptimes2
% 
% altered from rungetriptimes such that the days are included in the epoch,
% rather than in an outer m variable
%
% Is built from rungetripactivtimes.  Instead of returning the activation
% times, it returns a vector of all the times for any ripple.  This can 
% then be analyzed for distribution of event
% intervals and also for temporal correlations with behavioral events.
%
% still need to build function for returning times...
% build off getripactivetimes
...
PFsubsetlist = [{'firstonly'}, {'secondonly'}, {'intersect'}, {'neither'}]; % which PFincludeepochfilter to use for included place fields
daylist = 1:2; %1:5;
...
%param.day = day;
% description: text that will live in the output file to help identify it
describe = sprintf(['rungetriptimes2:  \n' ...
    'OUTPUT: is a list of times for ripples in which that cell \n' ...
    'was reactivated.  These times can be compared to events \n'...
    'i.e. freezing or air. \n'...
    'EPOCHS: denote epochs when ripples occur, filters 1 and 4 are sleep epochs. \n'...
    'filter 2 is shock track, filter 3 is ctrl track. \n'...
    'PLACE FIELDS: The two place field filters are 1.shock & 2.control. \n'...
    'PFSUBSET: designates logical set of place field filter \n'...
    'firstonly means cell had place field in first PFfilter, and not second. \n'...
    ]);
...
%mf = [];
%for m = daylist; % days = multiday   NOTE: this can be avoided by specifying multiple days directly in epochfilter
%    day = m;
for i = 1:length(stage)
    %i = stage(j);
    %day = 1;
    PFsubset = PFsubsetlist{stage(i)};
    ...
    epochfilter{1} = [ '(isequal($type, ''sleep'')) & (ismember($exposureday, [' num2str(daylist) '])) & $epoch == 1']; % isequal($environment 'sleep')
    epochfilter{2} = ['$shock == 1 & (ismember($exposureday, [' num2str(daylist) ']))'];
    epochfilter{3} = ['(isequal($environment, ''lineartrack'') | isequal($environment, ''ctrack'') ) & $shock == 0 & (ismember($exposureday, [' num2str(daylist) ']))'];
    epochfilter{4} = [ '(isequal($type, ''sleep'')) & (ismember($exposureday, [' num2str(daylist) '])) & $epoch == 4']; % isequal($environment 'sleep')
    ...
    PFepochfilter = [ '(isequal($type, ''run'')) & (ismember($exposureday, [' num2str(daylist) ']))'];
    ...
    iterator = 'multicellanal'; %'singlecellanal';
    ...
    %endepochfilter{1}{2} = [ '(isequal($type, ''sleep'')) & (ismember($exposureday, [' num2str(daylist) '])) & $epoch == 1']; % isequal($environment 'sleep')
    ...
    if minPeakPF > 0
        ...
        f{i} = excludecellsfsubset(f{i}, PFcells, 'includeepochfilter', PFincludeepochfilter, 'subset', PFsubset);
    end
    
    f{i}= setfilterfunction(f{i}, 'getrip', {'spikes','ripples','task','cellinfo'} , riptetfilter, 'minnumspikes', minnumspikes, 'appendindex', 1);%
    ...
    param.epochfilter = epochfilter;
    param.PFincludeepochfilter = PFincludeepochfilter;
    param.PFfiltersubset = PFsubset;
    for an = 1:length(animals)
        f{i}(an).scriptvars = param;
        f{i}(an).description = describe;
    end  
end
%mf{m} = f;
%end