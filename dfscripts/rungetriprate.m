...
maxmeanratePF = 5; % maximum mean rate of cells in reactivation epoch
ripthresh = 3; % currently does nothing, will pass to getriprate...
minriptet = 2;
maxvel = 4;  % 2 for original

rerunfiltfun = 1;
minfreeze = 100;%100;
mintime = 10;
plotgroups = [ 2 ]; % 1= shock track, 2=control
...
%NOT((vel<2)&(frz=0))
timefilter{2} = { {'get2dstate', ['((abs($velocity) <= ', num2str(maxvel), '))'] },...
           ...
           %maxtrodes = length(f{1}(an).speedeegdata{g}{d});
            %for tet = 1:maxtrodes;
                ...
                if (frztime{an}.shock(d)>minfreeze) %|| (frztime{an}.control(d)>minfreeze)
                    ...
                    if ismember(g,plotgroups) % shock track or control track
