...
for i = [1 2] % day
    ...
    epochfilter{i} = ['($exposureday == ',num2str(i),') & (isequal($type,''run'')) '];
end

ca1cellfilter = '(isequal($area, ''CA1'') && ($meanrate < 7))';
ca3cellfilter = '(isequal($area, ''CA3'') && ($meanrate < 7))';

% These time filters work with stress data
timefilter = { {'getestpos', '$freezing == 0'} };
%timefilter = { {'getlinstate', 'abs($velocity) >= 3', 6} };

% These time filters untested with stress data
