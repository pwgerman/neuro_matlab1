...
for an = 1:size(f,2) % test with only one animal
    for ee = 1:size(f(an).epochs{1},1)
        ...
        % correct for number of trials vs. total freezing events
        eventratio = size(unique(frzcoord(:,3)),1) / size(frzcoord(:,3),1);
        
        histin= histtimes .* histstate;
        histin = histin;
        histout = histc(histin, (0-rate):1:(40-rate)); % ?start index at rate?
        histom = mean(histout(2:end,:), 2);
        histom = histom*rate/eventratio;
        ...
        % ripplestart histogram
        histrip = histc(scoord(:,1), 0:1:40); % ?start index at rate?
        histrip = histrip/max(histrip);
        bar(1:1:size(histrip,1), histrip,1, 'EdgeColor', 'none', 'FaceColor', 'c'); alpha(.3);
        ylim('auto');
