% rungetripactivtimes
%
% Is built from rungetripactivprob.  Instead of returning the activation
% probability, it returns a vector of all the times that the given cell was
% reactivated.  This can then be analyzed for distribution of event
% intervals and also for temporal correlations with behavioral events.
%
% still need to build function for returning times...
% build off getripactiveprob
...
mf = [];
for m = 1 %1:5 % days = multiday   NOTE: this can be avoided by specifying multiple days directly in epochfilter
    day = m;
for i = 1:length(stage)
    ...
    f{i}= setfilterfunction(f{i}, 'getripactiv', {'spikes','ripples','task','cellinfo'} , riptetfilter, 'minnumspikes', minnumspikes, 'appendindex', 1);%
    ...
    for an = 1:length(animals)
        f{i}(an).rungetripactivtimes = param;
    end  
end
mf{m} = f;
end