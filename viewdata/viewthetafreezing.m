% viewthetafreezing
%   (was previously called viewfreeze_theta_eeg, and similar to
%   viewthetanoise) 
%   Plots the t2theta power (4-6Hz) along with freezing or speed.
%   To view freeze triggered windows for eegs use
    ...
    for day = daylist;%1:3
        for ep = eplist;%[2 3 5 7]
            ...
            for tt = 1:length(tetlist)
                ...
                rippleNT = load(loadstring); % old data
                tmprippleNT = rippleNT.ripple{day}{ep}{tet};
                ...
                xlim('auto');
                ylim('auto')
                plot(eegtimes(tmpt2theta), tmpt2theta.data(:,3), 'g'); % 1=filtered_amplitude, 3=envelope
                hold on;
                ...
                for jj = 1:length(riptimes)
                    plot([riptimes(jj) riptimes(jj)], [rastervert(jj,1) rastervert(jj, 2)], 'Color', hue('pink'));  % pink [1,0.4,0.6]
