function est_freeze_times = view_stop_est(ANIMPREFIX, DUR)
...
for half = .1%[.1 .2];
    e = 1;
    for epsilon = .5 %[.25 .5 .75 1] %.5%[.5 .6]
        d = 1;
        for duration = DUR; %[.5 1 1.5 2]
            time_diff = []; %initialize new matrix for holding difference between estimate and manual score
            for day = 1:4
                
                % assign stress on day one to epoch 2 or 3
                if strcmp(ANIMPREFIX,'Dic') | strcmp(ANIMPREFIX,'Eli') | strcmp(ANIMPREFIX,'Jig')
                    epoch.control = 2+rem(day,2); % alternate for each day; Cummings shock is epoch=3 on day=1
                    epoch.stress = 2+rem(day+1,2);
                else
                    epoch.stress = 2+rem(day,2); % alternate for each day; Bukowski, Cummings shock is epoch=3 on day=1
