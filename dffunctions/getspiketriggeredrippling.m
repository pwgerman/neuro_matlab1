function [out] = getspiketriggeredrippling(index, excludeperiods, ripples, spikes, varargin)
% [out] = GETSPIKETRIGGEREDRIPPLING(index, excludeperiods, ripples, spikes, varargin)
%
...
for i = 1:length(cells(:,1))
    cell = cells(i,:);
    if ~isempty(spikes{index(1,1)}{index(1,2)}{cell(1,1)}{cell(1,2)}.data)
        spiketimes = spikes{index(1,1)}{index(1,2)}{cell(1,1)}{cell(1,2)}.data(:,1);
        xcorrstruct = spikexcorr(ripstart, spiketimes, binsize, window(2));
