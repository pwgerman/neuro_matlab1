function out = dayprocess(daydirect, animdirect, fileprefix, daynum, varargin)

% dayprocess is a modification of standar dayprocess that can optionally
% take a cell array of inputs for CMPERPX that is equal in length to the
% number of 
% epochs.  Each entry in CMPERPX is used for the corresponding epoch.  The
% structure of cmperpix is: cmperpix{epoch}.
% Walter German 12-Aug-2013 10:47:49
%
%
% Note that wdayprocess2 is the same as wdayprocess as far as the cmperpix
% modification. The difference is that dayprocess2 is built on a later
% version of dayprocess with modifications made by Shantanu (line 212)
%
%
...
% check that cmperpix array is correct size.
%if length(cmperpix)~= 1 & length(cmperpix)~=length(times)
%    error('cmperpix must be 1 or array of all epochs');
%end
