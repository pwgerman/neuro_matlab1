% vieweeg
% use the zoom buttons on the matlab figures to zoom into areas of interest
% in the eeg and better see the short times at areas of interest.
...
for ripnum = 1:maxrip
    if ripples{ind(1)}{ind(2)}{ind(3)}.maxthresh(ripnum)>4
        ...
        if starttime - startbuf > ripples{ind(1)}{ind(2)}{ind(3)}.timerange(1)
            ...
            endtime = starttime + en% plot each ripple
maxrip = size(ripples{ind(1)}{ind(2)}{ind(3)}.starttime, 1);
for ripnum = 1:maxrip
    if ripples{ind(1)}{ind(2)}{ind(3)}.maxthresh(ripnum)>4
        startind = ripples{ind(1)}{ind(2)}{ind(3)}.startind(ripnum);
