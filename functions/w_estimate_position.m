...
posinfo = struct('params',params, 'units', 'centimeters', ...
  'fields', 'time x y dir vel xvel yvel xres yres stopped', 'data', []);
[position{:}] = deal(posinfo);

if isempty(epochs)
    epoch_list = 1:numel(rawpos);
else
    epoch_list = epochs;
end
...
for i = epoch_list
  ...
  if isscalar(centimeters_per_pixel)
      cm_per_pix = centimeters_per_pixel{1}{i};
  else
      cm_per_pix = centimeters_per_pixel(i)
  end
  ...
  n = numel(t);
  xfront = xfront * cm_per_pix;
  yfront = yfront * cm_per_pix;
  if (back_exists)
    xback = xback * cm_per_pix;
    yback = yback * cm_per_pix;
  end
